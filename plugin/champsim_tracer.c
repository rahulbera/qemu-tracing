 ...
 *
 * To decompress for inspection:
 *   zstd -d trace_vcpu0.raw.zst
 *   ./trace_inspector trace_vcpu0.raw
 *
 * ======================================================================
...
static inline uint32_t encode_header(uint8_t vcpu_id,
                                     uint8_t privilege,
                                     uint8_t instr_size,
                                     uint8_t num_mem_ops)
{
  return ((uint32_t)(vcpu_id & 0xF)) | ((uint32_t)(privilege & 0x1) << 4) |
         ((uint32_t)(instr_size & 0xF) << 5) |
         ((uint32_t)(num_mem_ops & 0x7) << 9);
}
...
typedef struct {
  ...
  uint64_t bytes_uncompressed; /* total raw bytes generated */
  uint64_t bytes_compressed;   /* total compressed bytes written */
  bool     active;
  bool     limit_reached;
} VcpuState;
 ...
 *
 * Takes the uncompressed input buffer, compresses it through zstd
 * streaming API, and writes the compressed output to disk.
 *
 * This is the ONLY place that does I/O. The hot path (buffer_append)
 * is a plain memcpy into the uncompressed buffer.
 * ================================================================ */
...
static void flush_buffer(VcpuState *vs)
{
  ...
  ZSTD_inBuffer input = {.src = vs->inbuf, .size = vs->inbuf_pos, .pos = 0};

  /* Compress in a loop (single pass is usually enough for our buffer
   * sizes, but the loop handles edge cases correctly) */
  while (input.pos < input.size) {
    ZSTD_outBuffer output = {.dst  = vs->outbuf,
                             .size = vs->outbuf_size,
                             .pos  = 0};

    size_t ret =
      ZSTD_compressStream2(vs->cctx, &output, &input, ZSTD_e_continue);
    if (ZSTD_isError(ret)) {
      fprintf(stderr,
              "[%s] ZSTD error: %s\n",
              PLUGIN_NAME,
...
/*
 * Final flush: tell zstd to finalize the stream (writes any
 * remaining compressed data and the end-of-frame marker).
 */
static void flush_final(VcpuState *vs)
{
  ...
  /* First flush any remaining uncompressed data */
  flush_buffer(vs);

  /* Then finalize the zstd stream */
  ...
  } while (ret > 0); /* ret > 0 means more output to flush */
}

/* Append data to the uncompressed input buffer */
...
static void finalize_pending_insn(VcpuState *vs)
{
  ...
  uint32_t header = encode_header(vs->vcpu_id,
  ...
  for (int i = 0; i < vs->pending_num_mem_ops; i++) {
    ...
    if (mop->flags & 0x2) {
      uint8_t value_bytes = (mop->size <= MAX_VALUE_SIZE) ? mop->size
                                                          : MAX_VALUE_SIZE;
...
static void plugin_atexit(qemu_plugin_id_t id, void *userdata)
{
  ...
  for (int i = 0; i < MAX_VCPUS; i++) {
    ...
    /* Finalize last instruction */
    finalize_pending_insn(vs);

    /* Finalize zstd stream (flushes remaining + writes end marker) */
    flush_final(vs);
  ...
  double total_ratio = total_comp > 0 ? (double)total_raw / total_comp : 0;

  fprintf(stderr,
          "[%s] Total: %" PRIu64 " insns, %" PRIu64 " mem ops, "
                                           ...
                                           char             **argv)
{
  ...
  fprintf(stderr,
          "[%s] ChampSim raw trace plugin v%d "
          "(values + zstd compression)\n",
          PLUGIN_NAME,
          TRACE_FORMAT_VER);
  fprintf(stderr,
          "[%s] QEMU target: %s, plugin API: %d (min %d)\n",
          PLUGIN_NAME,
  ...
  if (!info->system_emulation) {
    fprintf(stderr,
            "[%s] ERROR: requires system emulation mode\n",
            PLUGIN_NAME);
    return -1;
  }
  ...
  for (int i = 0; i < MAX_VCPUS; i++) {
    ...
    /* Allocate uncompressed input buffer */
    vs->inbuf     = g_malloc(INPUT_BUF_SIZE);
    vs->inbuf_pos = 0;

    /* Allocate compressed output buffer.
     * ZSTD_CStreamOutSize() gives the recommended output buffer size.
     */
    ...
    /*
     * Write the file header as the first bytes of the zstd stream.
     * The header goes through compression like everything else.
     * The decompressor (or trace_inspector on the decompressed file)
     * will see it as the first 16 bytes.
     */
  ...
  fprintf(stderr,
          "[%s] Initialized. Tracing begins on first insn exec.\n",
          PLUGIN_NAME);
