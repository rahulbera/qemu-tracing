...
int main(int argc, char **argv)
{
  ...
  /* Initialize Zydis decoder for 64-bit mode */
  ...
  while (1) {
    ...
    /* Decode x86 instruction with Zydis */
    ZydisDecodedInstruction insn;
    ZydisDecodedOperand     operands[ZYDIS_MAX_OPERAND_COUNT];

    ZyanStatus status = ZydisDecoderDecodeFull(
      &decoder, insn_bytes, isz, &insn, operands);

    if (ZYAN_SUCCESS(status)) {
      /* Branch classification */
      if (is_branch_instruction(&insn)) {
        rec.is_branch = classify_branch(&insn);
        stats.branch_insns++;
      }

      /* Instruction type */
      rec.instr_type = classify_instr_type(&insn);
        ...
        if (op->type == ZYDIS_OPERAND_TYPE_REGISTER) {
          ...
          if (op->actions & ZYDIS_OPERAND_ACTION_MASK_WRITE) {
            if (dst_reg_idx < NUM_INSTR_DESTINATIONS) {
              rec.destination_registers[dst_reg_idx++] = reg_id;
            }
          }
          if (op->actions & ZYDIS_OPERAND_ACTION_MASK_READ) {
            if (src_reg_idx < NUM_INSTR_SOURCES) {
              rec.source_registers[src_reg_idx++] = reg_id;
            }
          }
      ...
      if (rec.is_branch == 3 /* BRANCH_CONDITIONAL */) {
        bool has_flags = false;
        for (int i = 0; i < src_reg_idx; i++) {
          if (rec.source_registers[i] == 25) { has_flags = true; break; }
        }
        if (!has_flags && src_reg_idx < NUM_INSTR_SOURCES) {
          rec.source_registers[src_reg_idx++] = 25;
        }
      }

      /* For CALL/RET, ensure RSP is both source and destination */
      if (rec.is_branch == 4 || rec.is_branch == 5 || rec.is_branch == 6) {
        /* CALL or RET modifies RSP */
        bool has_rsp_src = false, has_rsp_dst = false;
        for (int i = 0; i < src_reg_idx; i++)
          if (rec.source_registers[i] == 6) has_rsp_src = true;
        for (int i = 0; i < dst_reg_idx; i++)
          if (rec.destination_registers[i] == 6) has_rsp_dst = true;
        if (!has_rsp_src && src_reg_idx < NUM_INSTR_SOURCES)
          rec.source_registers[src_reg_idx++] = 6;
        if (!has_rsp_dst && dst_reg_idx < NUM_INSTR_DESTINATIONS)
          rec.destination_registers[dst_reg_idx++] = 6;
      }

      /* For branch instructions, RIP is a destination */
      if (rec.is_branch && dst_reg_idx < NUM_INSTR_DESTINATIONS) {
        rec.destination_registers[dst_reg_idx++] = 26;
      }

      /* Stats */
      switch (rec.instr_type) {
      case INSTR_TYPE_FP:   stats.type_fp++; break;
      case INSTR_TYPE_SIMD: stats.type_simd++; break;
      default:              stats.type_int++; break;
      }
    } else {
      /* Decode failed — fill what we can */
